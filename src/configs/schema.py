...
class TrainingConfig:
    ...
    # Cross-entropy label smoothing factor (Szegedy et al., 2016). 0.0
    # disables smoothing (the original Hugging Face default); a small
    # positive value such as 0.1 typically yields a 0.2-0.4 pp gain in
    # F1-macro on AG News by reducing overconfidence on training examples.
    label_smoothing_factor: float = 0.0
    # R-Drop regularisation (Liang et al., 2021). When set to a positive
    # value, training runs two dropout-masked forward passes per batch
    # and adds a symmetric KL-divergence term between the two predicted
    # distributions, scaled by ``r_drop_alpha``. Set to 0.0 (default)
    # to disable, 1.0 to enable with the coefficient recommended by the
    # original paper.
