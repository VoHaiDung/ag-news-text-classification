...
# License : MIT
# ==============================================================================
"""Back-translation augmentation (target -> EN -> target).

WBS tasks 5A.2 and 5B.2 augment the target-language training set
(Vietnamese for Phase 5A, French for Phase 5B) by routing each
sentence through an English pivot. The aim is paraphrase-style
augmentation that preserves the topic label while introducing
surface variation.

The forward / backward OPUS-MT model names are configurable through
:class:`BackTranslationConfig` so the same pipeline drives VI -> EN ->
VI (``opus-mt-vi-en`` then ``opus-mt-en-vi``) and FR -> EN -> FR
(``opus-mt-fr-en`` then ``opus-mt-en-fr``).

The implementation reuses :class:`OpusMTTranslator` for both legs so
the quality and dependency footprint of the augmentation matches the
primary EN -> target translation step.
"""
