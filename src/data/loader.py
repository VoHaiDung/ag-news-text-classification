...
class AGNewsLoader:
    ...
    def _unify_columns(self, ds: DatasetDict) -> DatasetDict:
        ...
        A rename is only applied when (a) the source column is present,
        (b) the target name differs from the source, and (c) the target
        name is not already a column in the dataset. The third check
        prevents a duplicate-schema error when loading a dataset that
        was already produced by Phase 5A or 5B (which add a ``text_vi``
        or ``text_fr`` column side-by-side with the original ``text``
        column).
        """
        ...
        if (
            "text" in sample.column_names
            and self.config.text_column != "text"
            and self.config.text_column not in sample.column_names
        ):
            rename["text"] = self.config.text_column
        if (
            "label" in sample.column_names
            and self.config.label_column != "label"
            and self.config.label_column not in sample.column_names
        ):
