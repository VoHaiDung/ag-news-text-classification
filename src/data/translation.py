...
WBS tasks 5A.1 and 5B.1 require producing target-language copies of
AG News (Vietnamese for Phase 5A, French for Phase 5B) so that the
multilingual extension can be trained without relying on
target-native labelled corpora. The translator wraps a HuggingFace
``MarianMT`` model whose name is passed in via ``OpusMTConfig`` (for
example ``Helsinki-NLP/opus-mt-en-vi`` or
``Helsinki-NLP/opus-mt-en-fr``) and exposes a batched ``translate``
method that handles long inputs by chunking them on sentence
boundaries.
