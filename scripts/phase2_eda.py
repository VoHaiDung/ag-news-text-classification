...
def main() -> int:
    ...
    # Pre-compute text-length columns so visualisation helpers can read them
    # without re-running ``_to_dataframe`` on the full training split.
    train_df["text_length_chars"] = train_df[config.data.text_column].str.len()
    train_df["text_length_words"] = train_df[config.data.text_column].str.split().map(len)
